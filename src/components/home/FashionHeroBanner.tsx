...
export default function FashionHeroBanner() {
  ...
  // Fashion-specific banners
  const fashionBanners: BannerType[] = [
    {
      id: "fashion-1",
  ...
  const getCurrentSlideDuration = () => {
    const slide = fashionBanners[activeIndex];
  ...
  useEffect(() => {
    if (fashionBanners.length <= 1) return;
    ...
    const startSlideTimer = () => {
      ...
      timeoutRef = setTimeout(() => {
        ...
        setActiveIndex((current) => (current + 1) % fashionBanners.length);
      }, duration);
    };
  ...
  }, [activeIndex, fashionBanners.length, videoDurations]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const currentSlide = fashionBanners[activeIndex];
  ...
  }, [activeIndex, fashionBanners, showFloatingVideo]);
