...
export default function SportsHeroBanner() {
  ...
  // Sports-specific banners
  const sportsBanners: BannerType[] = [
    {
      id: "sports-1",
  ...
  const getCurrentSlideDuration = () => {
    const slide = sportsBanners[activeIndex];
  ...
  useEffect(() => {
    if (sportsBanners.length <= 1) return;
    ...
    const startSlideTimer = () => {
      ...
      timeoutRef = setTimeout(() => {
        ...
        setActiveIndex((current) => (current + 1) % sportsBanners.length);
      }, duration);
    };
  ...
  }, [activeIndex, sportsBanners.length, videoDurations]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const currentSlide = sportsBanners[activeIndex];
  ...
  }, [activeIndex, sportsBanners, showFloatingVideo]);
